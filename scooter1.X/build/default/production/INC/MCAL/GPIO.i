...
# 1 "INC/MCAL/GPIO.h" 1
# 15 "INC/MCAL/GPIO.h"
# 1 "INC/MCAL/../device_config.h" 1
# 15 "INC/MCAL/GPIO.h" 2
...
# 16 "INC/MCAL/GPIO.h" 2

# 1 "INC/MCAL/../std_types.h" 1
# 12 "INC/MCAL/../std_types.h"
# 1 "INC/MCAL/../compiler.h" 1
# 12 "INC/MCAL/../std_types.h" 2
...
Std_ReturnType gpio_pin_intilaize(const pin_config_t *pin){
   ...
   if (!(pin)||((pin->pin)>(8 -1)))
    ...
    { case INPUT:
        (*(tris_register[pin->port])|=(0X01<<pin->pin));
        break ;
  case OUTPUT:
    (*(tris_register[pin->port])&=~(0X01<<pin->pin));
      break ;
...
Std_ReturnType gpio_pin_get_direction_status(const pin_config_t *pin ,direction_t *status){
    ...
    }else {*status =((*tris_register[pin->port]>>(pin->pin))&0X01);
...
Std_ReturnType gpio_pin_write_logic(const pin_config_t *pin ,logic_t logic){
     ...
     if (!(pin))
         ...
         switch(logic)
         {
             case HIGH :(*lat_register[pin->port]|=(0X01<<pin->pin));
             break;
         case LOW :(*lat_register[pin->port]&=~(0X01<<pin->pin));
         break;
             default :ret=(Std_ReturnType)0x00;
         }
    } return ret ;
...
Std_ReturnType gpio_pin_read_logic(const pin_config_t *pin ,logic_t *status){
    ...
    }else {*status =((*port_register[pin->port]>>(pin->pin))&0X01);
...
Std_ReturnType gpio_pin_toggle_logic(const pin_config_t *pin ){
     ...
     (*lat_register[pin->port]^=(0X01<<pin->pin));
    } return ret ;
}
